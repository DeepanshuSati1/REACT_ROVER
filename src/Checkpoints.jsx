...
export default function Checkpoints({ checkpoint, setCheckpoint, isVolumeOn }) {
  ...
  const [checkpointSound] = useState(
    () => new Audio("./static/checkpoint.mp3")
  );
  ...
  useFrame(() => {
    ...
    const formattedTime = `${padWithZero(minutes)}:${padWithZero(
      seconds
  ...
  return (
    <>
      ...
      {phase !== "paused" ? (
        <Html
          wrapperClass={"circuit-stats-overlay"}
