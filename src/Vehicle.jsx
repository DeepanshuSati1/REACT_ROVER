...
import { Wheel } from "../../circuit-rush/src/Wheel.jsx";
...
export default function Vehicle({ checkpoint, performanceMode }) {
  ...
  useFrame((state, delta) => {
    ...
    const gearSpeedRange =
      currentGear === 0
        ? maxSpeeds[currentGear]
        : maxSpeeds[currentGear] - maxSpeeds[currentGear - 1];
    const speedWithinGearRange =
      currentGear === 0 ? speed : speed - maxSpeeds[currentGear - 1];
    ...
    setTransmission(
      reverseFlag ? "R" : gear === 0 ? 1 : gear === 6 ? "5" : gear.toString()
    );
    setGearProgress(
      speed === 1
        ? 2.5
  ...
  return (
    <>
      ...
      {phase !== "paused" ? (
        <Html
          wrapperClass={"vehicle-stats-overlay"}
        ...
        >
          ...
          <div className="vehicle-gear-stats">
            <div
              className="vehicle-speed-bar"
              style={{ width: `${gearProgressBar}%` }}
            ></div>
          </div>
      ...
      {resetAlert ? (
        <Html
          wrapperClass={"reset-alert-overlay"}
          className="reset-alert"
          ref={resetAlertContent}
        >
          <div ref={resetAlertMessage} className="reset-message">
