...
import useGame from "../../circuit-rush/src/stores/Game";
import waypointsLeft from "../../circuit-rush/public/static/waypoints-left";
import waypointsRight from "../../circuit-rush/public/static/waypoints-right";
...
export default function Objects({ cubesData, cubesCount, performanceMode }) {
  ...
  useEffect(() => {
    phase === "playing"
      ...
      : (enableCollisionHandlingRef.current = false);
  }, [phase]);
  ...
  return (
    <>
      ...
      {[waypointsRightData, waypointsLeftData].map(
        (waypointsData, sideIndex) => (
          <Instances
            key={sideIndex}
            range={waypointsData.length}
            material={objectsMaterial}
            geometry={
              sideIndex === 0
                ? waypointRight?.Waypoint?.geometry
                : waypointLeft?.Waypoint?.geometry
            }
            castShadow
          >
            <group position={[0, 0, 0]}>
      ...
      {[waypointsRightData, waypointsLeftData].map(
        (waypointsData, sideIndex) => (
          <Instances
            ...
            geometry={
              sideIndex === 0
                ? arrowRight?.Arrow?.geometry
                : arrowLeft?.Arrow?.geometry
            }
          >
            <group position={[0, 0, 0]}>
              {waypointsData.map((props, i) => (
                <Arrow
                  ...
                  refs={
                    sideIndex === 0
                      ? arrowRightInstanceRefs
                      : arrowLeftInstanceRefs
                  }
                />
              ))}
            </group>
